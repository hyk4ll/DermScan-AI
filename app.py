...
LOGO_PATH = BASE_DIR / "LogoGUI.jpg"
HERO_VIDEO_PATH = BASE_DIR / "mp_.mp4"
IMG_SIZE = (28, 28)
...
def render_specs_section():
    ...
    for condition in DISPLAY_CLASS_ORDER:
        ...
        description = truncate_text(info.get("description", "No description available."))
        cards.append(
            f'<div class="condition-card metric-card">'
            f'<div class="severity-pill {severity_class}">{severity}</div>'
            f'<h3>{condition}</h3>'
            f'<p>{description}</p>'
            f'</div>'
        )
